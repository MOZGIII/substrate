...
bitflags! {
	...
	struct CallFlags: u32 {
		...
		/// Allow the caller reentrancy into the current contract.
		///
		...
		const ALLOW_REENTRANCY = 0b0000_1000;
	}
}
		...
		UncheckedFrom<<E::T as frame_system::Config>::Hash> + AsRef<[u8]>
{
	...
	fn call(
		...
		if !flags.contains(CallFlags::ALLOW_REENTRANCY) && self.ext.is_executing(&callee) {
			Err(Error::<E::T>::ReentranceDenied)?;
		}
...
define_env!(Env, <E: Ext>,
	...
	[seal0] seal_terminate(
		...
		if ctx.ext.is_recursive() {
			Err(<Error<E::T>>::TerminatedWhileReentrant)?;
		}
